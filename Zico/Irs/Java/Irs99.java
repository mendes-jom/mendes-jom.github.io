...
public class Irs99 extends java.applet.Applet {
  ...
  int      IRS,DAC80,NOFIM,Zico,RL,DE,IRSPAGO,DRC55,ART55,COLECTAVEL,RB,IS,MPS_PDR ;
  int[][]  XXX      =new int[25][25];
  ...
  int[]    RBH0     =new int[6] ; // XXX[7][] //PENSOES
  int[]    DAH      =new int[6] ; // XXX[10][]//PENSOES
  ...
  int[]    RBX      =new int[6] ; // XXX[18][];
  int[]    RBY      =new int[6] ; // XXX[19][];
...
//	myString = new String("Zico: Cálculo de Irs... ");
  ...
  public void paint (Graphics g) {
   ...
   g.drawString("Isento       "+myNR(IS),mm,10+15*3);
   g.drawString("DEsp.        "+myNR(DE),mm,10+15*4);
   g.drawString("RL           "+myNR(RL),mm,10+15*5);

	   g.drawString("ART55        "+myNR(ART55),mm,10+15*6);//70
	   g.drawString("DRC55        "+myNR(DRC55),mm,10+15*7);//85
	   g.drawString("COLECTAVEL   "+myNR(COLECTAVEL),mm,10+15*8);//100
	   g.drawString("IRS-COLECTA  "+myNR(IRS),mm,10+15*9);//115
	   g.drawString("IRS PAGO     "+myNR(IRSPAGO),mm,10+15*10);//130
	

   g.drawString("DAC99        "+myNR(DAC99),mm,10+15*11);//145
   g.drawString("DAC80        "+myNR(DAC80),mm,10+15*12);//160
...
// Mais vale ter um \n ProgramaZico \nque não ter \nprograma nenhum...";
        ...
        int LimDiv,Educa,Seguro,Donat,Const,Quot
           ,LimIsRB,LimIsRBx,LimIsRBH0,LimIsRBH0x,LimDeRBH,LimDeRBH_D,LimDeRBH_ND;
        int Div,Renova,MaxDa,MaxDah,IS_H,IS_A;
  MPS_PDR=a;
  RL=-MPS_PDR;RB=0;RL=0;DE=0;IS=0;IS_A=0;IS_H=0;
  int CX=0;if (c>0) {CX=1;}
  OldAno=Ano;if  (ONtest==0)  { Ano=2;} // Spec 1999
	if (CX==0 ){     // c.equals("C")
...
LimIsRB =LimIsRB1[Ano];//1999
LimIsRBx=LimIsRB1x[Ano];//1999
LimDeRBH=LimDeRBH1[Ano];//1999

LimIsRBH0 =LimIsRBH01[Ano];//1999
LimIsRBH0x=LimIsRBH01x[Ano];//1999
LimDeRBH_ND=LimDeRBH1[Ano];
LimDeRBH_D =LimDeRBH1D[Ano];
Ano=OldAno;// Spec 99  

// ** ART55: a) -> j)
IRSPAGO=0;
int Div2=0,zQuots=0,TMP1=0;
int[] BaseQuots=new int[6] ;;
 ...
 // nao necessário:           			   RLA[j]=XXX[11][j];
		...
		DETI[j] =  XXX[15][j];      // NEW 99:DETI
                ...
                LimDeRBH=LimDeRBH_ND;
                   if (DEFICIENTE[j]==1)  {LimDeRBH=LimDeRBH_D;}                                                            
TMP1=0;
if (ONtest==0){
      MaxDa = 511865*(1+DEFICIENTE[j]*50/100);
//      MaxDah=1443300*(1+DEFICIENTE[j]*50/100);

  if ((ONtest==0) && (XXX[ 5][j]>0) && (DEFICIENTE[j]==0) )  
       {DA[j]   =   XXX[ 5][j]/XXX[ 5][j]*MaxDa;
        DA[j]=Math.min(DA[j],XXX[ 5][j]);
        DA[j]=XXX[ 5][j];
       }//simplificacão 99
/*
  if ((ONtest==0) && (XXX[7][j]>0)  )  
      {DAH[j] =  XXX[7][j]/XXX[7][j]*1443300;
       DAH[j]  =Math.min(DAH[j],XXX[7][j]);
       }//simplificacão 99

  if ((ONtest==0) && (XXX[7][j]>0) && (DEFICIENTE[j]==1) )  
       {DAH[j] +=  Math.min(XXX[7][j]*30/100,1415760);//1388000*1.02
        DAH[j]=Math.min(DAH[j],XXX[7][j]);
       }//simplificacão 99
*/
}
                ...
                RB     =RB     +RBX[j]+XXX[6][j]+RBH0[j]+XXX[14][j];//99:+TRTI
                IS_A     = DEFICIENTE[j]* Math.min(LimIsRBx*RBX[j]/100,LimIsRB);
                IS_H     = DEFICIENTE[j]* Math.min(LimIsRBH0x*RBH0[j]/100,LimIsRBH0);
                IS       = IS+ IS_A+IS_H;
                DAH[j]   = Math.min(  LimDeRBH,  RBH0[j]-IS_H);  
                DE     =DE     + DA[j]+DETI[j]+DAH[j];  //99:DA+DETI+DAH
                RL     =RB    -DE-IS;         
                ...
                Div2=Div2+RBA[j];
                if (Ano>1) { Div2=Div2+RBH0[j] ; BaseQuots[j]=BaseQuots[j]+RBH0[j];}
                                           }
...
public int[]    M1      ={1010000 ,1050000 ,1080000*Multip/100 , 700000};
public int[]    M2      ={2350000 ,2435000 ,2500000*Multip/100 ,1105000};
public int[]    M3      ={6000000 ,6150000 ,6280000*Multip/100 ,2750000}; //2560000};
...
public int[]    EducaN  ={                0, 20000  //(1997.. a mais pelo 3º,4º,.. filho
				           , 35000 , 35700};
//--Energia renovável:
public int[]    Renova1 ={262000-LimDiv1[0],30000  ,10000*Multip/100 ,25000};
public int[]    Renova2 ={422000-LimDiv1[0],60000  ,10000*Multip/100 ,25000};
...
public int[]    Const1  ={297000  , 305000 , 308000*Multip/100 ,314160};
public int[]    Const2  ={297000  , 305000 , 308000*Multip/100 ,314160};
...
public int[]    PPR1    ={400000  , 410000 , 418000*Multip/100 ,426360};
public int[]    PPR2    ={800000  , 820000 , 418000*2*Multip/100 ,852720};
public int[]    PPRx    ={    20  ,     20 ,     20 ,20};//0.2 : 20%

public int[]    PPH1    ={400000  , 410000 , 418000*Multip/100 ,426360};
public int[]    PPH2    ={400000  , 410000 , 418000*Multip/100 ,426360};
...
// Privatizações:
public int[]    PRI1    ={130000  , 130000 , 130000*Multip/100 ,132600};
public int[]    PRI2    ={261000  , 261000 , 261000*Multip/100 ,266220};
public int[]    PRIx    ={    20  ,     20 ,     20 ,20};//0.2 : 20%

public int[]    BES1    ={196000  , 196000 , 196000 *Multip/100,199920};
public int[]    BES2    ={391000  , 391000 , 391000 *Multip/100,398820};
...
public int[]    PPA1    ={150000  , 150000 , 150000*Multip/100 ,306000};
public int[]    PPA2    ={300000  , 300000 , 300000*Multip/100 ,306000};
...
public int[]    DAC80_1 ={33000    ,34500    ,  35200*Multip/100 ,36000};
public int[]    DAC80_2 ={25000*2  ,26300*2  ,  26800*2*Multip/100,27400*2};
public int[]    DAC80_AX={    0    ,19000    ,  19400*Multip/100  ,19800};
public int[]    DAC80_F1={ 18000   ,19000    ,  19400*Multip/100  ,19800};
public int[]    DAC80_F2={  +200   ,     +210,   +220*Multip/100  , +225};
public int[]    DAC80_F3={  +400   ,     +430,   +440*Multip/100  ,+450};
public int[]    DAC80_FX={  +500   ,     +550,   +560*Multip/100  ,+575};
//COMPUTADORES:(1998-2001):art 49-D do EBF
public int[]    COMP    ={ 0  , 0 , 30000 ,30000};//*=
public int[]    COMPx   ={ 0  , 0 ,    20 ,20};//%*=
//ACONSELHAMENTO JURÍDICO:(1998..)
public int[]    JURI    ={ 0  , 0 , 20000*Multip/100 ,25000};//*+
...
public int[]   LimIsRB1  ={0  , 0 , 2460000*Multip/100 ,2512000};//1999*
public int[]   LimIsRB1x ={0  , 0 ,  50 ,50};//1999*
public int[]   LimIsRBH01 ={0  , 0 , 1388000*Multip/100 ,1418000};//1999*
public int[]   LimIsRBH01x={0  , 0 ,  30 ,30};//1999
public int[]   LimDeRBH1 ={0  , 0 , 1415000*Multip/100 ,1445000};//1999*
public int[]   LimDeRBH1D={0  , 0 , 1839500*Multip/100 ,1878500};//1999*
